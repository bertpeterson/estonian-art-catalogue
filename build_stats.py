...
CSS = ("body{margin:0;padding:28px;font:15px/1.55 -apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;background:#fff;color:#111;max-width:940px}"
       ":root:not([data-theme=light]) body{background:#0b0c0e;color:#f1f2f4}:root:not([data-theme=light]) a{color:#9ec5ff}:root:not([data-theme=light]) th,:root:not([data-theme=light]) td{border-color:#2b2e34!important}:root:not([data-theme=light]) .m{color:#8b9098!important}"
       ...
       ":root:not([data-theme=light]) .hb .hr{fill:#f1f2f4}:root:not([data-theme=light]) .hb .hl,:root:not([data-theme=light]) .hb .hv{fill:#8b9098}"
...
def render(lang):
    ...
    parts = []
    parts = [f"<!doctype html><html lang=\"{lang}\"><head><meta charset=\"utf-8\"><script>try{{if(localStorage.getItem(\"ekr:theme\")===\"light\")document.documentElement.setAttribute(\"data-theme\",\"light\")}}catch(e){{}}</script><meta name=\"viewport\" content=\"width=device-width,initial-scale=1\">"
    ...
    parts.append(f"<p class=\"m\" style=\"margin-top:36px\">Auction results as Haus Galerii, Vernissage, Allee galerii, Vaal galerii, E-Kunstisalong and Eesti Kunsti Oksjonid published them, plus a few earlier record prices as the press reported them; kroon-era prices in euro at the fixed rate. A record, not a valuation. <a href=\"{HOME}\">{_('Full catalogue')}</a> · <a href=\"a/\">{_('Artists A–Z')}</a> · <a href=\"{OTHER}\">{OTHERLAB}</a></p><p class=\"m\">{_('Corrections, removals, questions:')} <a href=\"mailto:info@museaal.ee\">info@museaal.ee</a></p></body></html>")
